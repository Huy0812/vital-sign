...
class Facial_Skin(Resource) :
    ...
    def post(self):
        ...
        self.process.dark_circle(rule_roi, rarule_roi, rure_roi, rarure_roi)
        self.dark_circle = self.process.darkCircle

        print("________________", self.dark_circle)
        Infor_request['results']= {'dark circle': self.dark_circle}

        return jsonify(Infor_request['results'])
class Facial_HRate(Resource):
    ...
    def post(self):
        ...
        if(status_device == False):
            ...
            self.process = CaptureFrames()
            for i in all_buffer : 
                self.raw_bvp.append(i[1])
                self.arr_fps.append(i[3])
                self.process.raw_bvp_arr_forehead = self.raw_bvp
                self.process.fps = mean(self.arr_fps[-100:])
                self.process.vital_sign()
            ...
            indexes =[]
            for i in range (len(self.raw_bvp[100:])):
                indexes.append(i)
            plt.plot(indexes,self.raw_bvp[100:],'r')
            ...
            plt.savefig(f'./plot/{self.device_id}_facial_signal.png')
            plt.close()

            self.hr = self.process.heartrate
...
if __name__ == '__main__':
    ...
    api.add_resource(Facial_Skin, '/api/vitalsign/skin/v1.0')
